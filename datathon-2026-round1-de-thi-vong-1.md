...
| `discount_value` | float | Giá trị giảm (phần trăm hoặc số tiền tuỳ `promo_type`) |
...
- `F_i` là giá trị dự báo
- `A_i` là giá trị thực
- `Ā` là trung bình giá trị thực

- **MAE** đo độ lệch tuyệt đối trung bình
- **RMSE** phạt nặng hơn các sai số lớn
- **R²** thể hiện tỷ lệ phương sai được giải thích bởi mô hình

**Mục tiêu:**

- `MAE` và `RMSE` càng thấp càng tốt
...
3. **Khả năng giải thích (Explainability):** Trong report, bao gồm một mục giải thích các yếu tố dẫn động doanh thu chính được mô hình xác định (ví dụ: `feature importances`, `SHAP values`, hoặc `partial dependence plots`). Giải thích những gì mô hình học được bằng ngôn ngữ kinh doanh.
...
| Chất lượng trực quan hoá (15đ) | 8-12đ | Phần lớn biểu đồ đạt yêu cầu, một số thiếu nhãn hoặc chú thích |
| Chất lượng trực quan hoá (15đ) | 0-7đ | Biểu đồ thiếu thông tin, khó đọc hoặc không phù hợp với dữ liệu |
| Chiều sâu phân tích (25đ) | 21-25đ | Đạt cả bốn cấp độ Descriptive, Diagnostic, Predictive, Prescriptive một cách nhất quán |
| Chiều sâu phân tích (25đ) | 14-20đ | Đạt ba cấp độ, cấp độ Prescriptive còn hời hợt |
| Chiều sâu phân tích (25đ) | 7-13đ | Chủ yếu ở cấp Descriptive và Diagnostic |
| Chiều sâu phân tích (25đ) | 0-6đ | Chỉ mô tả bề mặt, thiếu phân tích |
| Insight kinh doanh (15đ) | 13-15đ | Đề xuất cụ thể, định lượng được, áp dụng được ngay |
| Insight kinh doanh (15đ) | 8-12đ | Có đề xuất nhưng còn chung chung |
| Insight kinh doanh (15đ) | 0-7đ | Thiếu kết nối với bối cảnh kinh doanh |
| Tính sáng tạo (5đ) | 4-5đ | Góc nhìn độc đáo, kết hợp nhiều nguồn dữ liệu, mạch trình bày thuyết phục |
| Tính sáng tạo (5đ) | 2-3đ | Có điểm sáng tạo nhưng chưa nhất quán |
| Tính sáng tạo (5đ) | 0-1đ | Phân tích dự đoán được, không có điểm nổi bật |
...
|---|---|---:|
| Hiệu suất mô hình | Dựa trên điểm `MAE`, `RMSE`, `R²` trên tập test (Kaggle leaderboard); xếp hạng tương đối so với các đội khác trong cuộc thi | 12 |
...
### Thang điểm chi tiết cho Phần 3

| Thành phần | Mức điểm | Mô tả |
|---|---|---|
| Hiệu suất mô hình (12đ) | 10-12đ | Xếp hạng top leaderboard; `MAE` và `RMSE` thấp, `R²` cao |
| Hiệu suất mô hình (12đ) | 5-9đ | Hiệu suất trung bình; mô hình hoạt động nhưng chưa tối ưu |
| Hiệu suất mô hình (12đ) | 3-4đ | Bài nộp hợp lệ nhưng hiệu suất thấp; mức điểm sàn |
...
| Báo cáo kỹ thuật (8đ) | 4-6đ | Pipeline đủ dùng, giải thích còn định tính, một số ràng buộc chưa được xử lý tường minh |
| Báo cáo kỹ thuật (8đ) | 0-3đ | Thiếu giải thích, không kiểm soát leakage, hoặc không thể tái lập kết quả |

**Điều kiện loại bài:** Bài nộp sẽ bị loại toàn bộ Phần 3 nếu vi phạm bất kỳ ràng buộc nào sau đây:
...
File này là bản chuyển đổi từ PDF sang Markdown để thuận tiện đưa vào repository, đọc trên GitHub và làm việc trong editor. Một số yếu tố dàn trang của PDF gốc đã được chuẩn hoá lại theo cấu trúc heading, danh sách và bảng Markdown.
